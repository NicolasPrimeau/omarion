...
async def test_emit_event(client):
    r = await client.post(
        "/events", json={"type": "custom.event", "payload": {"key": "val"}}, headers=HEADERS
    )
...
async def test_poll_events_filtered_by_type(client):
    ...
    r = await client.get(
        "/events",
        params={"since": "1970-01-01T00:00:00.000Z", "type": "want.this"},
        headers=HEADERS,
    )
    assert r.status_code == 200
    results = r.json()
...
async def test_poll_events_since_filters(client):
    ...
    r_all = await client.get(
        "/events", params={"since": "1970-01-01T00:00:00.000Z"}, headers=HEADERS
    )
    types_all = [e["type"] for e in r_all.json()]
    assert "first" in types_all
    assert "second" in types_all

    r_future = await client.get(
        "/events", params={"since": "2099-01-01T00:00:00.000Z"}, headers=HEADERS
    )
