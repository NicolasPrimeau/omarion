...
Embeddings generated on write using a local model (all-MiniLM-L6-v2 via sentence-transformers, runs on poseidon CPU). Stored in sqlite-vec. Used for:
...
Runs as a separate process on poseidon. Two trigger modes:
...
Replaces: poseidon-sync skill, docs/handoff/ files, JSONL parsing.
