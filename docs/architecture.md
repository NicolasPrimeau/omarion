...
Agents (any machine, any LLM framework)
  ↕  REST API  /  MCP
Omarion Server (server: ARTEL_HOST)
...
Docker Compose on server. `docker compose up -d` starts the server. SQLite data persists in a named volume at `/data/omarion.db`. The archivist runs as a second service in the same Compose file.
